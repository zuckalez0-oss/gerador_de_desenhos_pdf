...
class CodeGenerator:
    def __init__(self, db_path="codigo_database.xlsx"):
        ...
        # Nomes das colunas que serão usadas na planilha
        self.code_column_name = 'Codigo Unico'
        self.timestamp_column_name = 'Data de Registro'
        self.project_column_name = 'Projeto' # <-- NOVA COLUNA
    ...
    def _load_database(self):
        """Lê a planilha de códigos existente para encontrar o último número usado."""
        try:
            ...
            print(f"Arquivo de banco de dados '{self.db_path}' não encontrado. Um novo será criado.")
        except Exception as e:
            QMessageBox.warning(None, "Erro de Leitura", f"Não foi possível ler o banco de dados de códigos: {e}")

    def _append_to_database(self, new_code, project_number): # <-- RECEBE O NÚMERO DO PROJETO
        """Adiciona uma nova linha (código, timestamp, projeto) à planilha Excel."""
        timestamp = datetime.now().strftime('%d/%m/%Y %H:%M:%S')
        new_row = [new_code, timestamp, project_number] # <-- ADICIONA O PROJETO À LINHA
        ...
        try:
            ...
            wb = Workbook()
            sheet = wb.active
            # Cria o novo cabeçalho com a coluna do projeto
        ...
        try:
            wb.save(self.db_path)
             ...
             QMessageBox.critical(None, "Erro de Permissão", 
                                  f"Não foi possível salvar o novo código em '{self.db_path}'.\n\n"
                                  "Por favor, feche a planilha se ela estiver aberta e tente novamente.")
             return False
        except Exception as e:
    ...
    def generate_new_code(self, project_number, prefix='DES'): # <-- RECEBE O NÚMERO DO PROJETO
        """Gera um novo código, o anexa na planilha com o projeto e o retorna."""
        self._load_database()
        ...
        if self._append_to_database(new_code, project_number): # <-- PASSA O PROJETO PARA SALVAR
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # --- MUDANÇA: Adiciona o campo do projeto ---
        self.projeto_input = QLineEdit()
        manual_layout.addRow("Nº do Projeto:", self.projeto_input)

        self.nome_input = QLineEdit()
        generate_code_btn = QPushButton("Gerar Código")
        generate_code_btn.clicked.connect(self.generate_piece_code)
        
        name_layout = QHBoxLayout()
        name_layout.addWidget(self.nome_input)
        name_layout.addWidget(generate_code_btn)
        ...
        # ... (Restante do layout sem alterações) ...
    ...
    # --- MUDANÇA: Agora captura o projeto e o passa para o gerador ---
    def generate_piece_code(self):
        project_number = self.projeto_input.text().strip()
        if not project_number:
            QMessageBox.warning(self, "Campo Obrigatório", "Por favor, insira o 'Nº do Projeto' antes de gerar um código.")
            return

        new_code = self.code_generator.generate_new_code(project_number, prefix='DES')
        
        # Se new_code for None, significa que o salvamento falhou (ex: arquivo aberto)
        if new_code:
            self.nome_input.setText(new_code)
            self.log_text.append(f"Novo código '{new_code}' para o projeto '{project_number}' foi gerado e salvo.")

    # ... (Restante de todas as funções da MainWindow permanecem as mesmas) ...
    def update_table_display(self):
        combined_df = pd.concat([self.excel_df, self.manual_df], ignore_index=True)
        self.pieces_table.setRowCount(0)
        if combined_df.empty: self.process_btn.setEnabled(False); return
        self.pieces_table.setRowCount(len(combined_df))
        for i, row in combined_df.iterrows():
            for j, col in enumerate(self.colunas_df):
                value = row[col]
                if col == 'furos': display_value = f"{len(value)} Furo(s)" if isinstance(value, list) and value else '-'
                else: display_value = '-' if pd.isna(value) or value == 0 else str(value)
            ...
            edit_btn = QPushButton("Editar"); edit_btn.clicked.connect(lambda _, row=i: self.edit_row(row))
            delete_btn = QPushButton("Excluir"); delete_btn.clicked.connect(lambda _, row=i: self.delete_row(row))
        ...
        self.pieces_table.resizeColumnsToContents(); self.process_btn.setEnabled(True)
    def edit_row(self, row_index):
        len_excel = len(self.excel_df)
        if row_index < len_excel: piece_data = self.excel_df.iloc[row_index]; piece_name = piece_data['nome_arquivo']
        else: manual_index = row_index - len_excel; piece_data = self.manual_df.iloc[manual_index]; piece_name = piece_data['nome_arquivo']
        ...
        holes = piece_data.get('furos', []); self.furos_atuais = holes.copy() if isinstance(holes, list) else []; self.update_furos_table()
        if row_index < len_excel: self.excel_df = self.excel_df.drop(self.excel_df.index[row_index]).reset_index(drop=True)
        else: self.manual_df = self.manual_df.drop(self.manual_df.index[manual_index]).reset_index(drop=True)
        self.log_text.append(f"Peça '{piece_name}' carregada para edição. Adicione-a novamente após modificar."); self.update_table_display()
    def replicate_holes(self):
        try:
            forma = self.forma_combo.currentText()
            if forma != 'rectangle': QMessageBox.warning(self, "Função Indisponível", "A replicação de furos só está disponível para a forma 'Retângulo'."); return
