...
class HistoryManager:
    ...
    def save_project(self, project_number, pieces_df):
        ...
        history[project_number] = {"project_number": project_number, "save_date": datetime.now().strftime('%d/%m/%Y %H:%M:%S'), "pieces": pieces_df.to_dict('records')}
...
class HistoryManager:
    ...
    def save_project(self, project_number, pieces_df):
        history = self._load_history()
        # Garante que a coluna 'furos' seja convertida corretamente para o JSON
...
class HistoryDialog(QDialog):
    ...
    def populate_project_list(self):
        ...
        self.project_list_widget.addItems(self.history_manager.get_projects())
        ...
        if pieces:
            ...
            for i, piece in enumerate(pieces):
                for j, key in enumerate(headers):
                    value = piece.get(key)
                    # Formata a coluna de furos para ser mais legível
...
class ProcessThread(QThread):
    ...
    def __init__(self, dataframe_to_process, generate_pdf=True, generate_dxf=True, save_directory="."):
        ...
        try:
            ...
            if self.generate_pdf:
                ...
                for espessura, group in grouped:
                    pdf_filename = os.path.join(self.save_directory, f"Desenhos_PDF_Espessura_{str(espessura).replace('.', '_')}mm.pdf")
                    c = canvas.Canvas(pdf_filename, pagesize=A4)
                    for _, row in group.iterrows(): desenhar_forma(c, row); c.showPage()
                    c.save(); self.update_signal.emit(f"PDF '{pdf_filename}' gerado.")
            if self.generate_dxf:
                self.update_signal.emit("--- Gerando DXFs ---")
                zip_filename = os.path.join(self.save_directory, f"LOTE_DXF_{datetime.now():%Y%m%d_%H%M%S}.zip")
                with zipfile.ZipFile(zip_filename, 'w', zipfile.ZIP_DEFLATED) as zf:
                    for index, row in self.df.iterrows():
                        raw_data = row.to_dict()
                        prepared_data, error = dxf_engine.prepare_and_validate_dxf_data(raw_data)
                        if error: self.update_signal.emit(f"AVISO: Pulando DXF '{raw_data.get('nome_arquivo')}': {error}"); continue
...
# ==============================================
# ==============================================
# INTERFACE GRÁFICA (MAIN WINDOW) - VERSÃO FINAL COMPLETA
# ==============================================
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        file_button_layout.addWidget(self.history_btn)
        ...
        self.projeto_input = QLineEdit(); manual_layout.addRow("Nº do Projeto:", self.projeto_input)
        ...
        # Grupo 3: Furos
        furos_main_group = QGroupBox("3. Adicionar Furos"); furos_main_layout = QVBoxLayout()
        ...
        list_group = QGroupBox("4. Lista de Peças para Produção"); list_layout = QVBoxLayout()
        ...
        # Layout para o diretório de salvamento
        dir_layout = QHBoxLayout()
        self.dir_label = QLabel(f"Salvar em: {self.save_directory}"); self.dir_label.setStyleSheet("font-style: italic;")
        self.select_dir_btn = QPushButton("Selecionar Diretório...")
        dir_layout.addWidget(self.dir_label, 1); dir_layout.addWidget(self.select_dir_btn)
        list_layout.addLayout(dir_layout)
        
        # Layout para os botões de processo e conclusão
    ...
    def set_initial_button_state(self):
        ...
        self.select_file_btn.setEnabled(True); self.generate_code_btn.setEnabled(True); self.add_piece_btn.setEnabled(True)
        self.replicate_btn.setEnabled(True); self.add_furo_btn.setEnabled(True); self.history_btn.setEnabled(True)
        self.select_dir_btn.setEnabled(True)
        self.clear_excel_btn.setEnabled(False); self.process_pdf_btn.setEnabled(False)
        self.process_dxf_btn.setEnabled(False); self.process_all_btn.setEnabled(False)
        self.conclude_project_btn.setEnabled(False); self.progress_bar.setVisible(False)

    def set_buttons_enabled_on_process(self, enabled):
        """Controla o estado dos botões DURANTE o processamento."""
        self.select_file_btn.setEnabled(enabled); self.clear_excel_btn.setEnabled(enabled and not self.excel_df.empty)
        self.generate_code_btn.setEnabled(enabled); self.add_piece_btn.setEnabled(enabled)
        self.replicate_btn.setEnabled(enabled); self.add_furo_btn.setEnabled(enabled)
        self.history_btn.setEnabled(enabled); self.select_dir_btn.setEnabled(enabled)
    ...
    def select_save_directory(self):
        directory = QFileDialog.getExistingDirectory(self, "Selecionar Diretório para Salvar Arquivos", self.save_directory)
        if directory:
            self.save_directory = directory; self.dir_label.setText(f"Salvar em: {self.save_directory}")
            self.log_text.append(f"Diretório de salvamento definido como: {self.save_directory}")
    ...
    def start_processing(self, generate_pdf, generate_dxf):
        if not os.path.isdir(self.save_directory):
            QMessageBox.warning(self, "Diretório Inválido", "O diretório de salvamento não é válido."); return
        ...
        self.process_thread = ProcessThread(combined_df.copy(), generate_pdf, generate_dxf, self.save_directory)
    ...
    def show_history_dialog(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            if loaded_pieces:
                ...
                self.projeto_input.setText(str(project_number_loaded))
                self.excel_df = pd.DataFrame(columns=self.colunas_df)
                self.manual_df = pd.DataFrame(loaded_pieces)
                self.update_table_display()
                self.log_text.append(f"Projeto '{project_number_loaded}' carregado do histórico.")

    def conclude_project(self):
        project_number = self.projeto_input.text().strip()
        if not project_number:
            QMessageBox.warning(self, "Projeto sem Número", "Insira um 'Nº do Projeto' para salvá-lo no histórico.")
            return

        reply = QMessageBox.question(self, 'Concluir Projeto', f"Deseja salvar e concluir o projeto '{project_number}'?\n\nA lista será salva no histórico e depois limpa.", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        if reply == QMessageBox.Yes:
            combined_df = pd.concat([self.excel_df, self.manual_df], ignore_index=True)
            ...
            self._clear_all_input_forms(clear_project_number=True)
            self.excel_df = pd.DataFrame(columns=self.colunas_df)
            self.manual_df = pd.DataFrame(columns=self.colunas_df)
            self.update_table_display()
            self.log_text.append(f"\nPROJETO '{project_number}' CONCLUÍDO. Começando um novo.")
            self.statusBar().showMessage("Pronto para um novo projeto.")

    def _clear_all_input_forms(self, clear_project_number=False):
        """Função auxiliar para limpar todos os campos de entrada da interface."""
        ...
        for field in fields_to_clear:
            field.clear()
        self.furos_atuais = []; self.update_furos_table()
        self.file_label.setText("Nenhuma planilha selecionada")
    ...
    def add_manual_piece(self):
        try:
            ...
            self._clear_all_input_forms(clear_project_number=False)
            self.update_table_display()
        except ValueError: QMessageBox.critical(self, "Erro de Valor", "Campos numéricos devem conter números válidos.")
    
    def update_table_display(self):
        has_items = not (self.excel_df.empty and self.manual_df.empty)
        self.process_pdf_btn.setEnabled(has_items); self.process_dxf_btn.setEnabled(has_items)
        self.process_all_btn.setEnabled(has_items); self.conclude_project_btn.setEnabled(has_items)
    ...
    def generate_piece_code(self):
        project_number = self.projeto_input.text().strip()
        if not project_number: QMessageBox.warning(self, "Campo Obrigatório", "Insira o 'Nº do Projeto'."); return
