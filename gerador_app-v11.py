...
# Importa a engine de DXF (requer o arquivo dxf_engine.py na mesma pasta)
import dxf_engine
    ...
    def run(self):
        try:
            ...
            if self.generate_dxf:
                ...
                self.update_signal.emit(f"Arquivo ZIP salvo em: {zip_filename}")
...
class MainWindow(QMainWindow):
    ...
    def _clear_session(self, clear_project_number=False):
        ...
        if clear_project_number: # Limpa os DFs principais apenas em um reset completo
            self.excel_df = pd.DataFrame(columns=self.colunas_df)
            self.manual_df = pd.DataFrame(columns=self.colunas_df)
            self.update_table_display()
